...
def add_cathode_properties(data, wion):
    properties_dict = {
        ...
        "Volumetric capacity": {
            "Value[^val]": [r"{:.3f} mAh/L".format(data["Volumetric capacity (mAh/L)"].values[0])],
            "Model": [f"E(3)NN ({wion}-cathode)"],
        },
        "Gravimetric capacity": {
            "Value[^val]": [r"{:.3f} mAh/g".format(data["Gravimetric capacity (mAh/g)"].values[0])],
            "Model": [f"E(3)NN ({wion}-cathode)"],
        },
        "Volumetric energy": {
            "Value[^val]": [r"{:.3f} Wh/L".format(data["Volumetric energy (Wh/L)"].values[0])],
            "Model": [f"E(3)NN ({wion}-cathode)"],
        },
        "Gravimetric energy": {
            "Value[^val]": [r"{:.3f} Wh/kg".format(data["Gravimetric energy (Wh/kg)"].values[0])],
            "Model": [f"E(3)NN ({wion}-cathode)"],
        },
    }
...
<div class="admonition info"><p class="admonition-title"> Possible thermoelectric</p><p>
    ...
    as a potential thermoelectric material for working temperatures equal to {temp_list}, with a probability of """,
        ...
        for wT_ in ai_experts_m_dict.keys():
            info_.append(rf"{ai_experts_m_dict[wT_]*100.0:.2f} &#xb1; {ai_experts_dev_dict[wT_]*100.0:.2f} % @ {wT_} K")
        info["text"] += ", ".join(info_[:-1])
        info["text"] = info["text"] + f" and {info_[-1]}."
        info["text"] += r"""<sup id="fnref2:val"><a class="footnote-ref" href="#fn:val">1</a></sup></p></div>"""
